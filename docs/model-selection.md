...
A few Eden models are served on the OpenAI **Responses** API only and reject a chat-completions request with `Model(s) do not support /chat/completions` (OpenAI's `-pro` and `codex` families, in practice). Eden's catalogue does not publish which endpoint a model uses — there is no per-model route, the filter query parameters are ignored, and only one row in ~1000 carries an endpoint flag — so this is only discoverable by trying it. Kady therefore learns from the gateway: the error explains what happened and the model is dropped from the Eden AI list for the rest of the server's run, so it cannot be picked again. No id pattern is hardcoded, because "pro" is an ordinary tier name for Gemini, DeepSeek, and others whose chat endpoint works normally. A dropped model stays resolvable and correctly priced, so a ref pinned in a subagent file or `DEFAULT_MODEL_ID` still ledgers properly.
