...
- **Use [Eden AI](https://www.edenai.co) as a single gateway** with one API key — models from OpenAI, Anthropic, Google, Mistral, Qwen, Fireworks, and more. The model list is discovered live from your Eden account, and usage is billed per token, so it counts toward a project's spending cap. [Learn more](./docs/model-selection.md#eden-ai-models).
